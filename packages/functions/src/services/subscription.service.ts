...
import { PayPalConfig, AppConfig, GoogleConfig } from '../utils/config';
...
export async function createUserSubscription(
  ...
  appConfig: AppConfig
): Promise<{ approvalUrl: string; subscription: any; subscriptionId: string }> {
  // Validate inputs
  if (!userId || !serviceId) {
    throw new ValidationError('userId and serviceId are required');
  }
  ...
  if (!service.active) {
    throw new ValidationError(`Service '${serviceId}' is not available`);
  }

  if (!service.paypalPlanId) {
    throw new ValidationError(`Service '${serviceId}' does not have a PayPal plan configured`);
  }
  ...
  const links: Array<{ href: string; rel: string }> = subscription.links || [];
...
export async function activateUserSubscription(
...
): Promise<{ success: boolean; status: string; serviceId: string; subscription: any }> {
  ...
  if (mapping.userId !== userId) {
    throw new ValidationError('Subscription does not belong to this user');
  }
 ...
 */
async function activateSubscriptionForService(
  userId: string,
  serviceId: string,
  subscription: any,
  googleConfig: GoogleConfig
...
export async function cancelUserSubscription(
  ...
  paypalConfig: PayPalConfig,
  googleConfig: GoogleConfig,
  reason: string = 'Canceled by administrator'
): Promise<void> {
  const user = await FirestoreService.getUser(userId);
  const service = await FirestoreService.getService(serviceId);
  ...
  if (!userService) {
    throw new NotFoundError(`Subscription for service '${serviceId}'`);
  }
  ...
  if (userService.paymentMethod === 'paypal' && userService.paypalSubscriptionId) {
    ...
    await PayPalService.cancelSubscription(
      accessToken,
      paypalConfig,
      userService.paypalSubscriptionId,
      reason
    );
  }
 ...
 */
async function findUserByLegacySubscription(subscriptionId: string): Promise<any | null> {
...
export async function processWebhookEvent(
  event: any,
  googleConfig: GoogleConfig
): Promise<{ userId: string; serviceId: string } | null> {
