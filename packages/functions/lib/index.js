 ...
 */
app.get('/services/:serviceId', async (req, res, next) => {
    try {
 ...
 */
app.post('/create-subscription', async (req, res, next) => {
    try {
        const { uid, serviceId } = req.body;
        const paypalConfig = (0, config_1.getPayPalConfig)();
        const appConfig = (0, config_1.getAppConfig)();
        (0, config_1.validatePayPalConfig)(paypalConfig);
        const result = await SubscriptionService.createUserSubscription(uid, serviceId, paypalConfig, appConfig);
        res.json(result);
    }
    catch (error) {
        next(error);
 ...
 */
app.post('/activate-subscription', async (req, res, next) => {
    try {
        const { uid, subscriptionId } = req.body;
        const paypalConfig = (0, config_1.getPayPalConfig)();
        const googleConfig = (0, config_1.getGoogleConfig)();
        (0, config_1.validatePayPalConfig)(paypalConfig);
        const result = await SubscriptionService.activateUserSubscription(uid, subscriptionId, paypalConfig, googleConfig);
        res.json(result);
    }
    catch (error) {
        next(error);
 ...
 */
app.get('/user/subscription-status', async (req, res, next) => {
    try {
        let userEmail = null;
 ...
 */
app.delete('/admin/services/:serviceId', async (req, res, next) => {
    try {
        const { serviceId } = req.params;
 ...
 */
app.post('/admin/manual-payment', async (req, res, next) => {
    try {
        const { userEmail, serviceId, amount, durationMonths, paymentMethod, notes, receiptNumber, processedByUid, processedByEmail } = req.body;
        const googleConfig = (0, config_1.getGoogleConfig)();
 ...
 */
app.get('/admin/manual-transactions', async (req, res, next) => {
    try {
 ...
 */
app.get('/admin/manual-transactions/:userId', async (req, res, next) => {
    try {
 ...
 */
app.post('/admin/cancel-subscription', async (req, res, next) => {
    try {
        const { uid, serviceId, reason } = req.body;
        const paypalConfig = (0, config_1.getPayPalConfig)();
        const googleConfig = (0, config_1.getGoogleConfig)();
        await SubscriptionService.cancelUserSubscription(uid, serviceId, paypalConfig, googleConfig, reason);
 ...
 */
app.post('/admin/manage-group', async (req, res, next) => {
    try {
        ...
        const GoogleGroupsService = await Promise.resolve().then(() => __importStar(require('./services/google-groups.service')));
        let result;
        if (action === 'add') {
 ...
 */
app.post('/admin/migrate-users', async (req, res, next) => {
    try {
 ...
 */
app.post('/admin/seed-services', async (req, res, next) => {
    try {
        const SERVICES = [
            {
                id: 'music-library',
